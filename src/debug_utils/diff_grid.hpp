...
namespace debug_utils {
    ...
    std::string diff(
        const infrastructure::Grid<Dims, ElementType>& original,
        ...
        for (size_t i = 0; i < original.size(); i++) {
            if (original_data[i] != other_data[i]) {
                ...
                diff_str << "at: " << i << " ~ " << pretty(coords) << ": "
                    << std::to_string(original_data[i]) << " != \033[31m" << std::to_string(other_data[i]) << "\033[0m" << std::endl;
    ...
    std::string diff(const infrastructure::Grid<2, ElementType>& original, const infrastructure::Grid<2, ElementType>& other) {
        ...
        for (std::size_t y = 0; y < y_size; y++) {
            for (std::size_t x = 0; x < x_size; x++) {
                ...
                if (original[x][y] != other[x][y]) {
                    // diff_str << "\033[31m" << std::to_string(other[x][y]) << "\033[34m" << std::to_string(original[x][y]) << "\033[0m";
