...
void An5dAlg<32, ExecModel::CUDA>::run(An5dAlg<32, ExecModel::CUDA>::size_type iterations) {
    cuda_gol_32(input_bit_grid->data(), result_bit_grid->data(), input_bit_grid->x_size(), input_bit_grid->y_size(), iterations);
}

template <>
void An5dAlg<64, ExecModel::CUDA>::run(An5dAlg<64, ExecModel::CUDA>::size_type iterations) {
    cuda_gol_64(input_bit_grid->data(), result_bit_grid->data(), input_bit_grid->x_size(), input_bit_grid->y_size(), iterations);
}
